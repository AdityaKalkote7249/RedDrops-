...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // You should add your database connection and query logic here
    $email = $_POST['email'];
    $password = $_POST['password'];

    // Example: Assuming you have a 'users' table with email and password columns
    $host = "localhost";
    $username = "root";
    $dbpassword = "";
    $dbname = "bdm";
    ...
    $sql = "SELECT * FROM admin_add WHERE email='$email' AND passwor_d='$password'";
    $result = mysqli_query($conn, $sql);

    if (mysqli_num_rows($result) == 1) {
        // User found, store user data in session
        $_SESSION['user_data'] = mysqli_fetch_assoc($result);
        header("Location: dashboard.php"); // Redirect to the homepage
    } else {
    ...
    <div class="container">
        ...
        <form method="POST" action="">
            ...
            <input type="password" id="password" name="password" class="password" required
                style="width: 97%; height: 17px;" placeholder="1**8**12">
